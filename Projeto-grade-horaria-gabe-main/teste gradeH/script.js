...
// Armazenamento das grades horárias e avisos
let gradesHorarias = [];
let avisos = [];
let professoresFaltantes = [];
...
const btnPedagogos = document.getElementById('btn-pedagogos');
const btnAlunos = document.getElementById('btn-alunos');

// Função para carregar a seção de pedagogos
...
// Função para salvar a grade horária
...
// Função para salvar faltas de professores
...
// Função para carregar a seção de alunos
...
// Função para carregar a grade do aluno selecionado
...
btnPedagogos.addEventListener('click', () => {
    carregarAreaPedagogos();
    pedagogosSection.classList.remove('hidden');
...
btnAlunos.addEventListener('click', () => {
    ...
    pedagogosSection.classList.add('hidden');
});

// Inicializa a página mostrando a área dos alunos por padrão
carregarAreaAlunos();